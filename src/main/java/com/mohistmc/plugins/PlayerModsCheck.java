...
public class PlayerModsCheck {
    ...
    public static boolean init(List<String> stringList) {
        if (LunamuraConfig.server_modlist_whitelist_enable) {
            if (!ListUtils.is(Collections.singletonList(stringList), server_modlist_whitelist())) {
                canLog.set(false);
                return true;
            }
        }
        if (LunamuraConfig.player_modlist_blacklist_enable && LunamuraConfig.player_modlist_blacklist != null) {
            for (String config : LunamuraConfig.player_modlist_blacklist) {
    ...
    public static List<Object> server_modlist_whitelist() {
        String s = LunamuraConfig.server_modlist_whitelist.replaceAll("(?:\\[|null|\\]| +)", "");
