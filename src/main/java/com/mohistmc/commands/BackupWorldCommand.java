...
public class BackupWorldCommand extends Command {
    ...
    public BackupWorldCommand(String name) {
        ...
        this.setPermission("lunamura.command.backupworld");
    }
        ...
        if (sender.isOp()) {
            ...
            new Thread(() -> {
                try {
                    ...
                    File zip = new File("./LunamuraBackups/" + args[0] + "-" + now.getDayOfMonth() + "-" + now.getMonthValue() + "-" + now.getYear() + "-" + now.getHour() + "-" + now.getMinute() + "-" + now.getSecond() + ".zip");
                    ...
                    LunamuraMC.LOGGER.error("Failed to save world or this world doesn't exists.", e);
                }
            }).start();
        }
        return true;
