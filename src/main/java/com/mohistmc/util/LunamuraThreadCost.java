...
public class LunamuraThreadCost {
    ...
    public static void dumpThreadCpuTime() {
        ...
        LunamuraMC.LOGGER.info(LunamuraMC.i18n.as("lunamura.dump.1"));
        for (ThreadCpuTime threadCpuTime : list) {
            LunamuraMC.LOGGER.info(String.format("%s %s %s %s", threadCpuTime.id, threadCpuTime.name, threadCpuTime.cpuTime, threadCpuTime.userTime));
        }
    }
