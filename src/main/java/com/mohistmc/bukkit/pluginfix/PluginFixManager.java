    ...
    public static byte[] injectPluginFix(String className, byte[] clazz) {
        ...
        Consumer<ClassNode> patcher = switch (className) {
            ...
            case "com.sk89q.worldedit.bukkit.BukkitConfiguration" -> node -> {
                helloWorld(node, "I accept that I will receive no support with this flag enabled.", "lunamura");
                helloWorld(node, "allow-editing-on-unsupported-versions", "lunamura");
                helloWorld(node, "false", "lunamura");
            };
            case "net.Zrips.CMILib.Reflections" -> node -> helloWorld(node, "bR", "f_36096_");
            case "net.coreprotect.listener.ListenerHandler" -> node -> helloWorld(node, "net.kyori.adventure.text.serializer.plain.PlainTextComponentSerializer", "lunamura");
            default -> null;
        };
