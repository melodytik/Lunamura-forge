...
public class FarmlandWaterManager
{
    ...
    {
        if (DEBUG)
            LOGGER.info(LunamuraMC.i18n.as("lunamura.i18n.83", aabb));
        ...
        for (ChunkPos pos : posSet) //Find the chunkPos with the lowest distance to the center and choose it as the master pos
        {
            ...
            {
                if (DEBUG)
                    LOGGER.info(LunamuraMC.i18n.as("lunamura.i18n.84", masterPos, pos, masterDistance, distToCenter));
                masterPos = pos;
        ...
        if (DEBUG)
            LOGGER.info(LunamuraMC.i18n.as("lunamura.i18n.85", masterPos, posSet.toArray(new ChunkPos[0]), masterDistance));
    ...
    static void removeTickets(ChunkAccess chunk)
    {
        ...
        {
            if (DEBUG)
                LOGGER.info(LunamuraMC.i18n.as("lunamura.i18n.86", ticketManager.getTickets().size(), ticketManager.pos));
            ticketManager.getTickets().removeIf(next -> next.unload(ticketManager)); //remove if this is the master manager of the ticket
            if (DEBUG)
                LOGGER.info(LunamuraMC.i18n.as("lunamura.i18n.87", ticketManager.getTickets().size(), ticketManager.pos));
        }
    }
