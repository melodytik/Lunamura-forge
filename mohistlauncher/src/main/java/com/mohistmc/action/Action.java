...
public abstract class Action {
    ...
    public final String lunamuraVer;
    ...
    protected Action() {
        init();
        this.lunamuraVer = DataParser.versionMap.get("lunamura");
    ...
    protected void run(String mainClass, String[] args) throws Exception {
        ...
        System.out.println("[Lunamura] Loading " + classPath);
    ...
    protected void copyFileFromJar(File file, String pathInJar) {
        ...
        if (!file.exists() || !SHA256.is(file, SHA256.as(is)) || file.length() <= 1) {
            ...
            if (is != null) {
                ...
                System.out.println("[Lunamura] The file " + file.getName() + " doesn't exists in the Lunamura jar !");
                System.exit(0);
            }
        }
    }
    ...
    public boolean needsInstall() throws IOException {
        if (installInfo.exists()) {
            String jarmd = SHA256.as(LunamuraMCStart.jarTool.getFile());
